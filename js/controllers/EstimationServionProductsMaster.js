ReportApp.controller('EstimationServionProductsMasterController', function ($scope, $rootScope, _, $timeout, EstimationServionProductsMasterService, toaster, SelectionService, EstimationPageSectionService) {
    ...
    $scope.ServCollaorationApp = [];
    $scope.FineseBasedConfig = [];
    $scope.MobileConnector = [];
    $scope.MobileConnectorConfig = [];
    $scope.EnterpriseUCRoutingApp = [];
    $scope.EnterpriseUCRoutingAppConfig = [];
    $scope.ServIntuit = [];
    $scope.ServIntuitConfig = [];
    $scope.ServInsight = [];
    $scope.ServInsightConfig = [];
    $scope.Others = [];
    ...
    $scope.deleteRow = function () {
        ...
        if ($scope.ServCollaorationApp.length > 1 && $scope.ServCollaorationApp[rowid].IsHeader != 'Y') {
            $scope.ServCollaorationApp.splice(rowid, 1)
            $scope.Grid.api.setRowData($scope.ServCollaorationApp);
        }
        else {
    ...
    $scope.Addrow = function (GroupEditor, rowid) {
        $scope.ServCollaorationApp.splice(rowid, 0, GroupEditor)
    }
    ...
    var columnDefFirst = [
        ...
        {
            headerName: "", field: "Name1", width: 180, cellRenderer: TextEditor
        },
        {
    ...
    $scope.Grid = {
        ...
        columnDefs: [],
        groupHeaders: true,
        rowData: $scope.ServCollaorationApp,
    ...
    $scope.lastselected = { 'Rowid': 1, 'Gridname': 'ServCollaorationApp' };
    ...
    $scope.addFreshrows = function () {
        ...
        EstimationPageSectionService.GetAllPageSections().success(function (data) {
            ...
            EstimationServionProductsMasterService.GetAllEstimationServionProductsMaster().success(function (data) {
    ...
    $scope.addnewpricerow = function () {
        ...
        var GroupType = '';
        var data = $scope.ServCollaorationApp;
    ...
    $scope.savemaster = function () {
        ...
        if (filtered != null) {
            ...
            EstimationServionProductsMasterService.AddEstimationServionProductsMaster(filtered).success(function (data) {
                if (data.Error != undefined) {
                    toaster.pop('error', "Error", data.Error, null);
                } else {
