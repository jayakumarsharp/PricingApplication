ReportApp.controller('EstimationServionProductsController', function ($scope, $rootScope, _, EstimationServionProductsMasterService, toaster, $routeParams, $timeout, Opportunityservice, $window, $location, EstimationApplicationMasterService, SelectionService, EstimationPageSectionService) {
    ...
    $scope.EditSheet = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var LockSheetModel = { OppId: $scope.OpportunityDetail.OppId, GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'ServionProductsSheet', IsPriceSheetUpdated: true };

            EstimationServionProductsMasterService.LocktheSheetByGroupid(LockSheetModel).success(function (data) {
                ...
                if (!data.IsLocked) {
                    ...
                    if (data.LockedUser == $rootScope.UserInfo.user.userId && data.LockedIn == "ServionProductsSheet") {
                        $scope.hasedit = false;
                        $scope.griduseredit = true;
                        $scope.grideditable = true;
                        $routeParams.GroupId = data.ApplicationId;
    ...
    function getservicedata() {
        ...
        if ($routeParams.GroupId != 'undefined') {
            EstimationServionProductsMasterService.GetAllEstimationServionProductsbyOppGroupID($scope.OpportunityDetail.OppId, $scope.MaxSheetGroupID).success(function (data) {
                if (data.Error == null) {
    ...
    function gettypebsedfill(params) {
        angular.forEach(params, function (val, key) {
            ...
            localJson['A1Fcol1'] = gettype(val.A1Fcol1);
            localJson['A1Fcol2'] = gettype(val.A1Fcol2);
    ...
    var columnDefs = [
        ...
        {
            headerName: "",
            children: [{ headerName: "Servion Collaoration App", field: "Name", width: 420 }]
        },
        {
            headerName: 'PROD',
            children: [
    ...
    function calculateFinalTotal() {
        ...
        $scope.result1.push($scope.ServCollaorationApp)
    ...
    function setrowdatacall() {
        ...
        $scope.gridOptionsTop.rowData = $scope.data;
        onFloatingBottomCount();
    ...
    function addgrid(val) {
        ...
        for (var i = 2; i < val; i++) {
            ...
            var appendname = 'A' + i;
            columnDefs.push({ headerName: 'Application # ' + i, children: [{ headerName: "", field: appendname + "col1", width: 70, cellRenderer: NonPercentTypeEditor }, { headerName: "", field: appendname + "col2", width: 70, cellRenderer: NonPercentTypeEditor }, { headerName: "Hours", field: appendname + "Fcol3", width: 70, cellRenderer: NonPercentTypeEditor }] });

            columnDefs1.push({ headerName: "", field: appendname + "col1", width: 70, cellRenderer: NonPercentTypeEditor });
            columnDefs1.push({ headerName: "", field: appendname + "col2", width: 70, cellRenderer: NonPercentTypeEditor });
            columnDefs1.push({ headerName: "Hours", field: appendname + "Fcol3", width: 70, cellRenderer: NonPercentTypeEditor });
    ...
    $scope.addadditional = function (type) {
        ...
        columnDefs.push({ headerName: 'Application # ' + $scope.CurrentAdditionalGrid, children: [{ headerName: "", field: appendname + "col1", width: 70, cellRenderer: NonPercentTypeEditor }, { headerName: "", field: appendname + "col2", width: 70, cellRenderer: NonPercentTypeEditor }, { headerName: "Hours", field: appendname + "Fcol3", width: 70, cellRenderer: NonPercentTypeEditor }] });
    ...
    function callbacksave(isdiscard) {
        ...
        var Jsondata = { OppId: $scope.OpportunityDetail.OppId, IsSaveAs: false, ApplicationId: $scope.MaxSheetGroupID, Version: $scope.MaxVersion, ServionProductsSheet: filtered, Authour: $rootScope.UserInfo.user.userId, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: $scope.Totalresult, RootId: $routeParams.OppId, Content: $scope.content };
    ...
    function AddSheetInternalcall(Jsondata) {
        EstimationServionProductsMasterService.AddEstimationServionProducts(Jsondata).success(function (data) {
            ...
            if (data.Error == '' || data.Error == undefined || data.Error == null) {
                ...
                var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'SERVION_PRODUCTS', ApplicationId: $scope.MaxSheetGroupID, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                EstimationApplicationMasterService.dointernalpercentagecalculationForCiscoGroup(IPdata).success(function (data) {
                    ...
                    if (!$scope.forceupdate)
                        toaster.pop('success', "SAVE", 'ServionProducts Sheet Saved Successfully', 3000);
                    return true;
                }).error(function (error) {
    ...
    $scope.UpdatePriceSheetVersion = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var Jsondata = { ExisitingPriceGroupId: $scope.MaxSheetGroupID, OppId: $scope.OpportunityDetail.OppId, IsSaveAs: true, ApplicationId: $scope.MaxSheetGroupIDForSaveAs, IsReadOnly: $scope.grideditable, ServionProductsSheet: filtered, Comment: $scope.Comment, Authour: $rootScope.UserInfo.user.userId };
            Jsondata.Version = version;
            Jsondata.NumberofApp = $scope.CurrentAdditionalGrid
            EstimationServionProductsMasterService.AddEstimationServionProducts(Jsondata).success(function (data) {
                if (data.Error == '' || data.Error == undefined || data.Error == null) {
                    var newdata = $scope.Totalresult.concat($scope.result1);
                    var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'SERVION_PRODUCTS', ApplicationId: $scope.MaxSheetGroupIDForSaveAs, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                    EstimationApplicationMasterService.dointernalpercentagecalculationForCiscoGroup(IPdata).success(function (data) {
    ...
    $scope.IncreaseAdditionalTimeToSheet = function () {
        var LockSheetModel = { GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'ServionProductsSheet' };
        EstimationServionProductsMasterService.IncreaseAdditionalTimeToSheet(LockSheetModel).success(function (data) {
            if (!data.IsLocked) {
                $scope.hasedit = false;
                $scope.griduseredit = true;
    ...
    $scope.IgnoreChanges = function () {
        EstimationServionProductsMasterService.ReleaseSheetWhenExpired($scope.MaxSheetGroupID).success(function (data) {
            //alert(data)
            $scope.hasedit = true;
            $scope.griduseredit = false;
