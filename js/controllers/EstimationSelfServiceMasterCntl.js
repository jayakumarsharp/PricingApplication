ReportApp.controller('EstimationSelfServiceMasterController', function ($scope, $timeout, $rootScope, _, EstimationSelfServiceMasterService, toaster, SelectionService, EstimationPageSectionService) {
    ...
    $scope.deleteRow = function () {
        ...
        if ($scope.GridDialogModules.length > 1 && $scope.GridDialogModules[rowid].IsHeader != 'Y') {
            $scope.GridDialogModules.splice(rowid, 1)
            $scope.gridOptionsFixed.api.setRowData($scope.GridDialogModules);
        }
        else {
    ...
    $scope.Addrow = function (GroupEditor, rowid) {
        $scope.GridDialogModules.splice(rowid, 0, GroupEditor)
    }
    ...
    var columnDefFirst = [
        { headerName: "RowId", field: "RowId", width: 50 },
        {
            ...
            field: "Name", width: 530, cellRenderer: TextEditor
        },
        {
            ...
            children: [
                ...
                { headerName: "", field: "Fcol3", width: 420, cellRenderer: Editor },
            ]
        }
    ];
    ...
    $scope.gridOptionsFixed = {
        ...
        columnDefs: [],
        groupHeaders: true,
        rowData: $scope.GridDialogModules,
    ...
    $scope.lastselected = { 'Rowid': 1, 'Gridname': 'DialogModules' };
    ...
    $scope.addFreshrows = function () {
        ...
        EstimationPageSectionService.GetAllPageSections().success(function (data) {
            ...
            EstimationSelfServiceMasterService.GetAllEstimationSelfServiceMaster().success(function (data) {
    ...
    $scope.addnewpricerow = function () {
        ...
        var data = $scope.GridDialogModules;
    ...
    $scope.savemaster = function () {
        ...
        if (filtered != null) {
            ...
            EstimationSelfServiceMasterService.AddEstimationSelfServiceMaster(filtered).success(function (data) {
                if (data.Error != undefined) {
                    toaster.pop('error', "Error", data.Error, null);
                } else {
