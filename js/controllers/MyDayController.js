ReportApp.controller('MyDayController', function ($scope, $rootScope, OppFactory, MyDayFactory, BillingConfigService , UserFactory, HolidayFactory, toaster, reportFactory, $timeout) {
    $scope.date = {};
    $scope.HRMIS = {};
    $scope.opp = {};
    $scope.Users = [];
    ...
    $scope.showSummaryTotal = false;
    $scope.TotalHoursSpent = '';
    $scope.TotalSummaryEfforts = '';
        ...
        HolidayFactory.GetHoliday(LoggedInUser.LocationId, min, max).success(function (holiday) {
            MyDayFactory.GetLockedDate().success(function (data) {
                ...
                angular.forEach($scope.MyTasks, function (value, key) {
                    ...
                    if ((value.TaskTypeId == 30 || value.TaskTypeId == 32) && IsDateHoliday(value.WorkDate, holiday)) {
                        console.log('Ignoring task as it falls on holiday');
                        toaster.pop('warning', "Warning", "Leave or No Work cannot be submitted on a Holiday", null);
                        isWorkDateValid = false;
                    }
                    else if (value.TaskTypeId == 30 && (value.hhDuration != 4 && value.hhDuration != 8)) {
                        toaster.pop('warning', "Warning", "Invalid hours entered for Leave task", null);
                        isWorkDateValid = false;
                    }
    ...
    $scope.GetTaskummaryReportTotal = function () {
        $scope.TaskSummary = [];
        $scope.showSummaryTotal = false;
        $scope.TotalSummaryEfforts = '';
        ...
        if ($scope.taskSummary.SBUId == undefined || $scope.taskSummary.SBUId.length == 0) {
            ...
            $scope.taskSummary.SBUId.push(0);
        }
        ...
        if (ValidateFromToDates($scope.taskSummary.FromDate, $scope.taskSummary.ToDate)) {
            MyDayFactory.GetTaskummaryReportTotal($scope.taskSummary).success(function (data) {
                if (data.length > 0)
                    $scope.TotalSummaryEfforts = data[0].TotalEfforts;
                else
                    $scope.TotalSummaryEfforts = 0;
                $scope.showSummaryTotal = true;
            }).error(function (error) {
                $scope.Error = error;
            });
        }
    ...
    var taskDetailsColumnDefs = [
        ...
        {
            headerName: "Opportunity ID", field: "OppId", width: 255, editable: false, headerTooltip: "OpportunityID", cellStyle: { 'text-align': 'center', 'display': 'flex', 'align-items': 'center' },
            // headerCellTemplate: function () {
            //     return '<div style="color:white"><b>Opportunity <br>ID</b></div>';
            // }
        },
        {
            headerName: "Opportunity Name", field: "OpportunityName", width: 260, editable: false, headerTooltip: "Opportunity Name", cellStyle: { 'text-align': 'left', 'display': 'flex', 'align-items': 'center' },
            // headerCellTemplate: function () {
            //     return '<div style="color:white"><b>Opportunity <br>Name</b></div>';
            // }
        },
        {
            headerName: "Opportunity SBU", field: "SBU", width: 255, editable: false, headerTooltip: "Opportunity SBU", cellStyle: { 'text-align': 'left', 'display': 'flex', 'align-items': 'center' },
            // headerCellTemplate: function () {
    ...
    $scope.GetUsersForMyDay = function () {
        UserFactory.GetInactiveUsers().success(function (inactiveusers) {
            UserFactory.GetUserHierarchy($rootScope.UserInfo.user.userId).success(function (data) {
                ...
                if ($scope.IsSCUser) {
                    console.log('Adding Inactive users to list for SC Head..');
                    angular.forEach(inactiveusers, function (val, key) {
                        $scope.UsersForMyDay.push(val.userId);
                    });
                }
        ...
        if ($scope.scUtilWeekInput.FromDate == undefined) {
            ...
            if (userid == undefined || userid == '') {
                ...
                angular.forEach($scope.Users, function (value, key) {
                    if (value.LastWorkingDate == null)
                        users.push({ 'UserId': value.userId, 'LocationId': value.LocationId });
                    else {
                        var last = moment.utc(value.LastWorkingDate).toDate();
                        var lwd = moment(last).format('L');

                        var localTime = moment.utc($scope.scUtilWeekInput.FromDate).toDate();
                        var frm = moment(localTime).format('L');

                        frm = moment(frm).subtract(3, 'days').format('L');
                        var to = moment(frm).add(6, 'days').format('L');
    ...
    function UpdateSCUtilsGrid() {
        ...
        angular.forEach($scope.SCUtilization, function (value, key) {
            ...
            var fmdate = new Date(moment($scope.scUtilInput.FromDate)), yf = fmdate.getFullYear(), mf = fmdate.getMonth();
            var todate = new Date(moment($scope.scUtilInput.ToDate)), yt = todate.getFullYear(), mt = todate.getMonth();
            var fromDay = new Date(yf, mf, 1);
            var toDay = new Date(yt, mt + 1, 0);

            fromDay = moment(fromDay).format('L');
            toDay = moment(toDay).format('L');

            console.log('Format with js: \n');
            console.log('From: ' + fromDay);
            ...
            var weekenddays = moment().weekdayCalc(fromDay, toDay, leavedays);


            var d1 = moment(fromDay);
            var d2 = moment(toDay);
            var totalDays = d2.diff(d1, 'days') + 1;

            var workingDays = totalDays - (weekenddays + Holidayavail); // Total Working Days
            var calcdate = workingDays - (value.MDLeaveDays == undefined ? 0 : value.MDLeaveDays);
    ...
    function UpdateSCBillingGrid() {
        ...
        try {
            ...
            MyDayFactory.GetUsersBillingID($scope.scBillingInput).success(function (mdusers) {
                MDUsers = mdusers;
    ...
    $scope.FetchSCBillingNew = function (typeid) {
        ...
        UserFactory.GetUsersByTypes(allTypes).success(function (data) {
            ...
            BillingConfig = [];
            BillingConfigService.GetBillingConfig($scope.scBillingNewInput.BillingPeriod).success(function (billconf) {
                if (billconf.length > 0) {
                    BillingConfig = billconf;
                    ...
                    toaster.pop('warning', "Warning", "Provide Billing configuration for this financial year!", null);
                }
            }).error(function (error) {
                $scope.Error = error;
            });
    ...
    function GetBillingValue(month, sbu, userid) {
        try {
            ...
            if (allSBUEfforts.length > 0) {
                ...
                if (allSBUEfforts[0].BillingId == 1) { //Shared Roles
                    if (buAllEffort > 0 && totalEffort > 0) {
                        var billParam = _.findWhere(BillingConfig, { SBU: sbu });
                        var mon = months[month - 1].toUpperCase();
                        if (billParam != undefined && billParam[mon] > 0)
                            calcValue += (buAllEffort / totalEffort) * (billParam[mon]);
                        else {
                            console.log('Billing parameter unavailable!!!');
                            calcValue = 0;
                        }
                    }
                }
                    ...
                    var buCnt = 1 / BU.length;
                    var buFound = false;
                    for (var i = 0; i < BU.length; i++) {
                        if (!buFound && parseInt(BU[i]) == sbu) {
                            calcValue += ((buAllEffort / totalEffort)) * buCnt;
                            buFound = true;
                        }
                    }
                }
                calcValue = calcValue * 100;
                calcValue = calcValue == 0 ? calcValue : calcValue.toFixed(2);
            }
            return calcValue + '%';
        }
        catch (ex) {
            console.log('ERROR occurred at GetBillingValue: ' + ex);
            return '0%';
        }
    };
...
ReportApp.factory('MyDayFactory', function ($http) {
    ...
    MyDayFactory.GetTaskummaryReportTotal = function (taskSummary) {
        var result = $http.post(MyDayURL + '/GetTaskummaryReportTotal', taskSummary);
        return result;
    }
