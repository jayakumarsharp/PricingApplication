ReportApp.controller('EstimationSelfServiceController', function ($scope, $rootScope, _, EstimationSelfServiceMasterService, toaster, $routeParams, $timeout, Opportunityservice, $window, $location, EstimationApplicationMasterService, SelectionService, $route, EstimationPageSectionService) {
        ...
        EstimationApplicationMasterService.GetEstimationApplicationVersionsForOpp(oppid, 'SelfServiceSheet').success(function (data) {
            ...
            _.each($scope.Versiondata, function (value, key) {
                if (value["ApplicationId"] == $scope.GlobalGroupId) {
                    ...
                    $scope.originalcount = $scope.CurrentAdditionalGrid;
    ...
    $scope.EditSheet = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var LockSheetModel = { OppId: $scope.OpportunityDetail.OppId, GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'SelfServiceSheet', IsPriceSheetUpdated: true };

            EstimationSelfServiceMasterService.LocktheSheetByGroupid(LockSheetModel).success(function (data) {
                ...
                if (!data.IsLocked) {
                    ...
                    if (data.LockedUser == $rootScope.UserInfo.user.userId && data.LockedIn == "SelfServiceSheet") {
                        $scope.hasedit = false;
                        $scope.griduseredit = true;
                        $scope.grideditable = true;
                        $routeParams.GroupId = data.ApplicationId;
    ...
    function getservicedata() {
        ...
        if ($routeParams.GroupId != 'undefined') {
            EstimationSelfServiceMasterService.GetAllEstimationSelfServicebyOppGroupID($scope.OpportunityDetail.OppId, $scope.MaxSheetGroupID).success(function (data) {
                if (data.Error == null) {
    ...
    var columnDefs = [
        ...
        {
            headerName: "Number of Number of IVR Call Flows : 1", children: [{ headerName: "", field: "Name", width: 520, },]
        },

        {
            headerName: 'IVR Call Flow # 1',
            children: [
                { headerName: "DM Count", field: "A1col1", width: 60, cellRenderer: NonPercentTypeEditor },
    ...
    function calculateFinalTotal() {
        ...
        $scope.result1.push($scope.GridDialogModules)
        ...
        result.push({
            Section: 'Total', Name: 'Total Estimated Hours', A1col1: '', A1col2: '', A1Fcol3: parseFloat(caljson.A1Fcol3).toFixed(2), A2col1: '', A2col2: '', A2Fcol3: parseFloat(caljson.A2Fcol3).toFixed(2), A3col1: '', A3col2: '', A3Fcol3: parseFloat(caljson.A3Fcol3).toFixed(2), A4col1: '', A4col2: '', A4Fcol3: parseFloat(caljson.A4Fcol3).toFixed(2), A5col1: '', A5col2: '', A5Fcol3: parseFloat(caljson.A5Fcol3).toFixed(2), A6col1: '', A6col2: '', A6Fcol3: parseFloat(caljson.A6Fcol3).toFixed(2), A7col1: ''
            , A7col2: '', A7Fcol3: parseFloat(caljson.A7Fcol3).toFixed(2), A8col1: '', A8col2: '', A8Fcol3: parseFloat(caljson.A8Fcol3).toFixed(2)
            , A9col1: '', A9col2: '', A9Fcol3: parseFloat(caljson.A9Fcol3).toFixed(2), A10col1: '', A10col2: '', A10Fcol3: parseFloat(caljson.A10Fcol3).toFixed(2)
        });
    ...
    $scope.addFreshrows = function (data) {
        ...
        EstimationPageSectionService.GetAllPageSections().success(function (datasection) {
            ...
            var filteredsections = _.filter($scope.GridSections, function (obj) {
                return obj.PageName == "SelfService";
            })
    ...
    function firstrunformula() {

        for (var j = 0; j < $scope.data.length; j++) {
            try {
                if ($scope.data[j].Fcol3 != undefined && $scope.data[j].Fcol3 != null && $scope.data[j].Fcol3 != '') {
                    for (var i = 1; i < $scope.CurrentAdditionalGrid; i++) {
                        try {
                            var formula = RunthroughFormula($scope.data[j].Fcol3, i);
                            $scope.data[j]['A' + i + 'Fcol3'] = eval(formula);
                            $scope.data[j]['A' + i + 'Fcol3'] = parseFloat($scope.data[j]['A' + i + 'Fcol3']).toFixed(2);
                            $scope.data[j]['A' + i + 'Fcol3'] = ($scope.data[j]['A' + i + 'Fcol3']).toString();
                        }
                        catch (e)
                        { alert(e); }
                    }
    ...
    function forscopelevel(params, count) {
    ...
    getcelldata = function (rownum, colname) {
        try {
    ...
    function addgrid(val) {
        ...
        for (var i = 2; i < val; i++) {
            ...
            var appendname = 'A' + i;
            columnDefs.push({
                headerName: 'IVR Call Flow # ' + i, children: [{ headerName: "", field: appendname + "col1", width: 60, cellRenderer: NonPercentTypeEditor }, { headerName: "", field: appendname + "col2", width: 60, cellRenderer: NonPercentTypeEditor }, {
    ...
    $scope.addadditional = function (type) {
        if ($scope.CurrentAdditionalGrid <= 10) {
            ...
            columnDefs.push({ headerName: 'IVR Call Flow # ' + $scope.CurrentAdditionalGrid, children: [{ headerName: "", field: appendname + "col1", width: 60, cellRenderer: NonPercentTypeEditor }, { headerName: "", field: appendname + "col2", width: 60, cellRenderer: NonPercentTypeEditor }, { headerName: "Hours", field: appendname + "Fcol3", width: 60, cellRenderer: NonPercentTypeEditor }] });
    ...
    function callbacksave(isdiscard) {
        ...
        var Jsondata = { OppId: $scope.OpportunityDetail.OppId, IsSaveAs: false, ApplicationId: $scope.MaxSheetGroupID, Version: $scope.MaxVersion, SelfServiceSheet: filtered, Authour: $rootScope.UserInfo.user.userId, NumberofApp: $scope.CurrentAdditionalGrid, Content: Content };
    ...
    function AddSheetInternalcall(Jsondata) {
        EstimationSelfServiceMasterService.AddEstimationSelfService(Jsondata).success(function (data) {
            $scope.content = '';
            ...
            if (data.Error == '' || data.Error == undefined || data.Error == null) {
                ...
                var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'SELF_SERVICE', ApplicationId: $scope.MaxSheetGroupID, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                EstimationApplicationMasterService.dointernalpercentagecalculation(IPdata).success(function (data) {
                    ...
                    if (!$scope.forceupdate)
                        toaster.pop('success', "SAVE", 'SelfService Sheet Saved Successfully', 3000);
                    return true;
                }).error(function (error) {
    ...
    $scope.UpdatePriceSheetVersion = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var Jsondata = { ExisitingPriceGroupId: $scope.MaxSheetGroupID, OppId: $scope.OpportunityDetail.OppId, IsSaveAs: true, ApplicationId: $scope.MaxSheetGroupIDForSaveAs, IsReadOnly: $scope.grideditable, SelfServiceSheet: filtered, Comment: $scope.Comment, Authour: $rootScope.UserInfo.user.userId };
            Jsondata.Version = version;
            Jsondata.NumberofApp = $scope.CurrentAdditionalGrid
            EstimationSelfServiceMasterService.AddEstimationSelfService(Jsondata).success(function (data) {
                if (data.Error == '' || data.Error == undefined || data.Error == null) {
                    var newdata = $scope.Totalresult.concat($scope.result1);
                    var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'SELF_SERVICE', ApplicationId: $scope.MaxSheetGroupIDForSaveAs, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                    EstimationApplicationMasterService.dointernalpercentagecalculation(IPdata).success(function (data) {
                        $scope.griduseredit = false;
                        redirectRefreshPage($routeParams.OppId, $routeParams.GroupId);
                    }).error(function (error) {
    ...
    $scope.IncreaseAdditionalTimeToSheet = function () {
        var LockSheetModel = { GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'SelfServiceSheet' };
        EstimationSelfServiceMasterService.IncreaseAdditionalTimeToSheet(LockSheetModel).success(function (data) {
            if (!data.IsLocked) {
                $scope.hasedit = false;
                $scope.griduseredit = true;
    ...
    $scope.IgnoreChanges = function () {
        EstimationSelfServiceMasterService.ReleaseSheetWhenExpired($scope.MaxSheetGroupID).success(function (data) {
            //alert(data)
            $scope.hasedit = true;
            $scope.griduseredit = false;
