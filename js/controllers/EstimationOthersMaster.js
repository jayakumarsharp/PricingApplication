ReportApp.controller('EstimationOthersMasterController', function ($scope, $timeout, $rootScope, _, EstimationOthersMasterService, toaster, SelectionService, EstimationPageSectionService) {
    ...
    $scope.deleteRow = function () {
        ...
        if ($scope.Servers.length > 1 && $scope.Servers[rowid].IsHeader != 'Y') {
            $scope.Servers.splice(rowid, 1)
            $scope.gridOptionsFixed.api.setRowData($scope.Servers);
        }
        else {
    ...
    $scope.Addrow = function (GroupEditor, rowid) {
        $scope.Servers.splice(rowid, 0, GroupEditor)
    }
    ...
    $scope.columnDefFirst1 = [
        ...
        { headerName: "Servers", field: "Name", width: 530, cellRenderer: TextEditor },
        { headerName: "# of Units", field: "col1", width: 60, cellRenderer: Editor },
    ...
    $scope.lastselected = { 'Rowid': 1, 'Gridname': 'Servers' };
    ...
    function callbackgrid() {
        $scope.columnDefFirst1[1].headerName = 'Servers';
    ...
    $scope.addFreshrows = function () {
        ...
        EstimationPageSectionService.GetAllPageSections().success(function (data) {
            ...
            EstimationOthersMasterService.GetAllEstimationOthersMaster().success(function (data) {
    ...
    $scope.savemaster = function () {
        ...
        if (filtered != null) {
            ...
            EstimationOthersMasterService.AddEstimationOthersMaster(filtered).success(function (data) {
                if (data.Error != undefined) {
                    toaster.pop('error', "Error", data.Error, null);
                } else {

                    toaster.pop('success', "Success", "Estimation Other Master added successfully", null);
                }
            }).error(function (data) {
                toaster.pop('error', "Error", "An Error has occured while Adding Estimation Application Master!", null);
                $scope.error = "An Error has occured while Adding Estimation Application Master! " + data.ExceptionMessage;
