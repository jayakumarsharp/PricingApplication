ReportApp.controller('EstimationAvayaController', function ($scope, $rootScope, _, EstimationAvayaMasterService, toaster, $routeParams, $timeout, Opportunityservice, $window, $location, EstimationApplicationMasterService, SelectionService, EstimationPageSectionService) {
    ...
    $scope.EditSheet = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var LockSheetModel = { OppId: $scope.OpportunityDetail.OppId, GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'AvayaSheet', IsPriceSheetUpdated: true };

            EstimationAvayaMasterService.LocktheSheetByGroupid(LockSheetModel).success(function (data) {
                ...
                if (!data.IsLocked) {
                    ...
                    if (data.LockedUser == $rootScope.UserInfo.user.userId && data.LockedIn == "AvayaSheet") {
                        $scope.hasedit = false;
                        $scope.griduseredit = true;
                        $scope.grideditable = true;
                        $routeParams.GroupId = data.ApplicationId;
    ...
    function getservicedata() {
        ...
        if ($routeParams.GroupId != 'undefined') {
            EstimationAvayaMasterService.GetAllEstimationAvayabyOppGroupID($scope.OpportunityDetail.OppId, $scope.MaxSheetGroupID).success(function (data) {
                if (data.Error == null) {
    ...
    var columnDefs = [
        ...
        {
            headerName: "",
            children: [{ headerName: "", field: "Name", width: 420 }]
        },
        {
            headerName: 'PROD',
            children: [
                { headerName: "", field: "A1col1", width: 70, cellRenderer: NonPercentTypeEditor },
            ]
        },
        {
            headerName: 'UAT',
            children: [
                { headerName: "", field: "A1col2", width: 70, cellRenderer: NonPercentTypeEditor },
            ]
        },
        {
            headerName: 'UPG',
            children: [
                { headerName: "", field: "A1col3", width: 70, cellRenderer: NonPercentTypeEditor }
            ]
        },
        ...
        },
        {
            ...
            children: [
                {
                    headerName: "", field: "A1Fcol1", width: 70, cellRenderer: function (params) {
                        if (params.value > 0) {
        ...
        },
        {
            ...
            children: [
                {
                    headerName: "", field: "A1Fcol2", width: 70, cellRenderer: function (params) {
                        if (params.value > 0) {
                            return '<span title=' + params.value + '>' + parseFloat(params.value).toFixed(2) + '</span>';
                        }
                        else {
        ...
        },
        {
            ...
            children: [
                {
                    headerName: "", field: "A1Fcol3", width: 70, cellRenderer: function (params) {
                        if (params.value > 0) {
                            return '<span title=' + params.value + '>' + parseFloat(params.value).toFixed(2) + '</span>';
                        }
                        else {
    ...
    $scope.addadditional = function (type) {
        if ($scope.CurrentAdditionalGrid < 7) {
            ...
            columnDefs.push({ headerName: 'Application # ' + $scope.CurrentAdditionalGrid, children: [{ headerName: "", field: appendname + "col1", width: 100, cellRenderer: NonPercentTypeEditor }, { headerName: "", field: appendname + "col2", width: 100, cellRenderer: NonPercentTypeEditor }, { headerName: "Hours", field: appendname + "Fcol3", width: 100, cellRenderer: NonPercentTypeEditor }] });
    ...
    function callbacksave(isdiscard) {
        ...
        var Jsondata = { OppId: $scope.OpportunityDetail.OppId, IsSaveAs: false, ApplicationId: $scope.MaxSheetGroupID, Version: $scope.MaxVersion, AvayaSheet: filtered, Authour: $rootScope.UserInfo.user.userId, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: $scope.Totalresult, RootId: $routeParams.OppId, Content: $scope.content };
    ...
    function AddSheetInternalcall(Jsondata) {
        EstimationAvayaMasterService.AddEstimationAvaya(Jsondata).success(function (data) {
            ...
            if (data.Error == '' || data.Error == undefined || data.Error == null) {
                ...
                var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'Avaya', ApplicationId: $scope.MaxSheetGroupID, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                EstimationApplicationMasterService.dointernalpercentagecalculationForCiscoGroup(IPdata).success(function (data) {
                    ...
                    if (!$scope.forceupdate)
                        toaster.pop('success', "SAVE", 'Avaya Sheet Saved Successfully', 3000);
                    return true;
    ...
    $scope.UpdatePriceSheetVersion = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var Jsondata = { ExisitingPriceGroupId: $scope.MaxSheetGroupID, OppId: $scope.OpportunityDetail.OppId, IsSaveAs: true, ApplicationId: $scope.MaxSheetGroupIDForSaveAs, IsReadOnly: $scope.grideditable, AvayaSheet: filtered, Comment: $scope.Comment, Authour: $rootScope.UserInfo.user.userId };
            Jsondata.Version = version;
            Jsondata.NumberofApp = $scope.CurrentAdditionalGrid
            EstimationAvayaMasterService.AddEstimationAvaya(Jsondata).success(function (data) {
                if (data.Error == '' || data.Error == undefined || data.Error == null) {
                    var newdata = $scope.Totalresult.concat($scope.result1);
                    var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'Avaya', ApplicationId: $scope.MaxSheetGroupIDForSaveAs, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                    EstimationApplicationMasterService.dointernalpercentagecalculationForCiscoGroup(IPdata).success(function (data) {
                        $scope.griduseredit = false;
                        redirectRefreshPage($routeParams.OppId, $routeParams.GroupId);
                    }).error(function (error) {
    ...
    $scope.IncreaseAdditionalTimeToSheet = function () {
        var LockSheetModel = { GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'AvayaSheet' };
        EstimationAvayaMasterService.IncreaseAdditionalTimeToSheet(LockSheetModel).success(function (data) {
            if (!data.IsLocked) {
                $scope.hasedit = false;
                $scope.griduseredit = true;
    ...
    $scope.IgnoreChanges = function () {
        EstimationAvayaMasterService.ReleaseSheetWhenExpired($scope.MaxSheetGroupID).success(function (data) {
            $scope.hasedit = true;
            $scope.griduseredit = false;
            $scope.grideditable = false;
            if ($scope.Isleaving)
