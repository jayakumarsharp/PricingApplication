ReportApp.controller('EstimationWFOController', function ($scope, $rootScope, _, EstimationWFOMasterService, toaster, $routeParams, $timeout, Opportunityservice, $window, $location, EstimationApplicationMasterService, SelectionService, EstimationPageSectionService) {
    ...
    $scope.EditSheet = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var LockSheetModel = { OppId: $scope.OpportunityDetail.OppId, GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'WFOSheet', IsPriceSheetUpdated: true };

            EstimationWFOMasterService.LocktheSheetByGroupid(LockSheetModel).success(function (data) {
                ...
                if (!data.IsLocked) {
                    ...
                    if (data.LockedUser == $rootScope.UserInfo.user.userId && data.LockedIn == "WFOSheet") {
                        $scope.hasedit = false;
                        $scope.griduseredit = true;
                        $scope.grideditable = true;
                        $routeParams.GroupId = data.ApplicationId;
    ...
    function getservicedata() {
        ...
        if ($routeParams.GroupId != 'undefined') {
            EstimationWFOMasterService.GetAllEstimationWFObyOppGroupID($scope.OpportunityDetail.OppId, $scope.MaxSheetGroupID).success(function (data) {
                if (data.Error == null) {
    ...
    var columnDefs = [
        ...
        {
            headerName: "",
            children: [{ headerName: "Nice", field: "Name", width: 420, editable: true }]
        },
        {
            headerName: 'PROD',
            children: [
    ...
    function calculateFinalTotal() {
        ...
        $scope.result1.push($scope.Nice);

        var data = $scope.data;

        var caljson = { A1Fcol1: 0, A1Fcol2: 0, A1Fcol3: 0 };
        ...
        result.push({
            Section: 'Total', Name: 'Total Estimated Hours', A1col1: '', A1col2: '', A1col3: '', A1Fcol1: caljson.A1Fcol1, A1Fcol2: caljson.A1Fcol2, A1Fcol3: caljson.A1Fcol3
        });
    ...
    function callbacksave(isdiscard) {
        ...
        var Jsondata = { OppId: $scope.OpportunityDetail.OppId, IsSaveAs: false, ApplicationId: $scope.MaxSheetGroupID, Version: $scope.MaxVersion, WFOSheet: filtered, Authour: $rootScope.UserInfo.user.userId, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: $scope.Totalresult, RootId: $routeParams.OppId, Content: $scope.content };
    ...
    function AddSheetInternalcall(Jsondata) {
        EstimationWFOMasterService.AddEstimationWFO(Jsondata).success(function (data) {
            ...
            if (data.Error == '' || data.Error == undefined || data.Error == null) {
                ...
                var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'WFO', ApplicationId: $scope.MaxSheetGroupID, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                EstimationApplicationMasterService.dointernalpercentagecalculationForEgainGroup(IPdata).success(function (data) {
                    ...
                    if (!$scope.forceupdate)
                        toaster.pop('success', "SAVE", 'WFO Sheet Saved Successfully', 3000);
                    return true;
                }).error(function (error) {
    ...
    $scope.UpdatePriceSheetVersion = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var Jsondata = { ExisitingPriceGroupId: $scope.MaxSheetGroupID, OppId: $scope.OpportunityDetail.OppId, IsSaveAs: true, ApplicationId: $scope.MaxSheetGroupIDForSaveAs, IsReadOnly: $scope.grideditable, WFOSheet: filtered, Comment: $scope.Comment, Authour: $rootScope.UserInfo.user.userId };
            ...
            EstimationWFOMasterService.AddEstimationWFO(Jsondata).success(function (data) {
                if (data.Error == '' || data.Error == undefined || data.Error == null) {
                    ...
                    var newdata = $scope.Totalresult.concat($scope.result1);
                    var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'WFO', ApplicationId: $scope.MaxSheetGroupIDForSaveAs, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                    EstimationApplicationMasterService.dointernalpercentagecalculationForEgainGroup(IPdata).success(function (data) {
                        $scope.griduseredit = false;
                        redirectRefreshPage($routeParams.OppId, $routeParams.GroupId);
                    }).error(function (error) {
    ...
    $scope.IncreaseAdditionalTimeToSheet = function () {
        var LockSheetModel = { GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'WFOSheet' };
        EstimationWFOMasterService.IncreaseAdditionalTimeToSheet(LockSheetModel).success(function (data) {
            if (!data.IsLocked) {
                $scope.hasedit = false;
                $scope.griduseredit = true;
    ...
    $scope.IgnoreChanges = function () {
        EstimationWFOMasterService.ReleaseSheetWhenExpired($scope.MaxSheetGroupID).success(function (data) {
            //alert(data)
            $scope.hasedit = true;
            $scope.griduseredit = false;
