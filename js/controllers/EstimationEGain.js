ReportApp.controller('EstimationEGainController', function ($scope, $rootScope, _, EstimationEGainMasterService, toaster, $routeParams, $timeout, Opportunityservice, $window, $location, EstimationApplicationMasterService, SelectionService, EstimationPageSectionService) {
    ...
    $scope.EditSheet = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var LockSheetModel = { OppId: $scope.OpportunityDetail.OppId, GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'EGainSheet', IsPriceSheetUpdated: true };

            EstimationEGainMasterService.LocktheSheetByGroupid(LockSheetModel).success(function (data) {
                ...
                if (!data.IsLocked) {
                    ...
                    if (data.LockedUser == $rootScope.UserInfo.user.userId && data.LockedIn == "EGainSheet") {
                        $scope.hasedit = false;
                        $scope.griduseredit = true;
                        $scope.grideditable = true;
                        $routeParams.GroupId = data.ApplicationId;
    ...
    function getservicedata() {
        ...
        if ($routeParams.GroupId != 'undefined') {
            EstimationEGainMasterService.GetAllEstimationEGainbyOppGroupID($scope.OpportunityDetail.OppId, $scope.MaxSheetGroupID).success(function (data) {
                if (data.Error == null) {
    ...
    var columnDefs = [
        ...
        {
            headerName: "",
            children: [{ headerName: "eGain", field: "Name", width: 420 }]
        },
        {
            headerName: 'PROD',
            children: [
    ...
    function calculateFinalTotal() {
        ...
        $scope.result1.push($scope.eGain);
    ...
    function callbacksave(isdiscard) {
        ...
        var Jsondata = { OppId: $scope.OpportunityDetail.OppId, IsSaveAs: false, ApplicationId: $scope.MaxSheetGroupID, Version: $scope.MaxVersion, EGainSheet: filtered, Authour: $rootScope.UserInfo.user.userId, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: $scope.Totalresult, RootId: $routeParams.OppId, Content: $scope.content };
    ...
    function AddSheetInternalcall(Jsondata) {
        EstimationEGainMasterService.AddEstimationEGain(Jsondata).success(function (data) {
            ...
            if (data.Error == '' || data.Error == undefined || data.Error == null) {
                ...
                var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'eGain', ApplicationId: $scope.MaxSheetGroupID, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                EstimationApplicationMasterService.dointernalpercentagecalculationForEgainGroup(IPdata).success(function (data) {
                    ...
                    if (!$scope.forceupdate)
                        toaster.pop('success', "SAVE", 'EGain Sheet Saved Successfully', 3000);
                    return true;
                }).error(function (error) {
    ...
    $scope.UpdatePriceSheetVersion = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var Jsondata = { ExisitingPriceGroupId: $scope.MaxSheetGroupID, OppId: $scope.OpportunityDetail.OppId, IsSaveAs: true, ApplicationId: $scope.MaxSheetGroupIDForSaveAs, IsReadOnly: $scope.grideditable, EGainSheet: filtered, Comment: $scope.Comment, Authour: $rootScope.UserInfo.user.userId };
            Jsondata.Version = version;
            Jsondata.NumberofApp = $scope.CurrentAdditionalGrid
            EstimationEGainMasterService.AddEstimationEGain(Jsondata).success(function (data) {
                if (data.Error == '' || data.Error == undefined || data.Error == null) {
                    var newdata = $scope.Totalresult.concat($scope.result1);
                    var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'eGain', ApplicationId: $scope.MaxSheetGroupIDForSaveAs, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                    EstimationApplicationMasterService.dointernalpercentagecalculationForEgainGroup(IPdata).success(function (data) {
                        $scope.griduseredit = false;
                        redirectRefreshPage($routeParams.OppId, $routeParams.GroupId);
                    }).error(function (error) {
    ...
    $scope.IncreaseAdditionalTimeToSheet = function () {
        var LockSheetModel = { GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'EGainSheet' };
        EstimationEGainMasterService.IncreaseAdditionalTimeToSheet(LockSheetModel).success(function (data) {
            if (!data.IsLocked) {
                $scope.hasedit = false;
                $scope.griduseredit = true;
    ...
    $scope.IgnoreChanges = function () {
        EstimationEGainMasterService.ReleaseSheetWhenExpired($scope.MaxSheetGroupID).success(function (data) {
            //alert(data)
            $scope.hasedit = true;
            $scope.griduseredit = false;
