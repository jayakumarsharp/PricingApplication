ReportApp.controller('EstimationCiscoMasterController', function ($scope, $rootScope, _, EstimationCiscoMasterService, toaster, SelectionService, $timeout, EstimationPageSectionService) {
    ...
    $scope.deleteRow = function () {
        ...
        if ($scope.UCSServers.length > 1 && $scope.UCSServers[rowid].IsHeader != 'Y') {
            $scope.UCSServers.splice(rowid, 1)
            $scope.Grid.api.setRowData($scope.UCSServers);
        }
        else {
    ...
    $scope.Addrow = function (GroupEditor, rowid) {
        $scope.UCSServers.splice(rowid, 0, GroupEditor)

    }

    var columnDefFirst = [
        { headerName: "RowId", field: "RowId", width: 50 },
        { headerName: "IsHeader", field: "IsHeader", width: 50 },
        {
            headerName: "", field: "Name", width: 335, cellRenderer: TextEditor
        },
        {
    ...
    $scope.lastselected = { 'Rowid': 1, 'Gridname': 'UCSServers' };
    ...
    $scope.addFreshrows = function () {
        ...
        EstimationPageSectionService.GetAllPageSections().success(function (data) {
            ...
            EstimationCiscoMasterService.GetAllEstimationCiscoMaster().success(function (data) {
    ...
    $scope.addnewpricerow = function () {
        ...
        if ($scope.lastselected.Gridname != '') {
            var freshrow = { 'GroupType': $scope.lastselected.Gridname, 'Name': 'Type name', 'Name1': '', 'col1': 'Text', 'col2': 'Text', 'col3': 'Text', 'Fcol1': '', 'Fcol2': '', 'Fcol3': '', 'RowId': $scope.currentmax, IsHeader: '', IsOriginal: true };
    ...
    $scope.savemaster = function () {
        ...
        if (filtered != null) {
            EstimationCiscoMasterService.AddEstimationCiscoMaster(filtered).success(function (data) {
                if (data.Error != undefined) {
                    toaster.pop('error', "Error", data.Error, null);
                } else {

                    toaster.pop('success', "Success", "Estimation Cisco Master added successfully", null);
                }
            }).error(function (data) {
                toaster.pop('error', "Error", "An Error has occured while Adding Estimation Application Master!", null);
                $scope.error = "An Error has occured while Adding Estimation Application Master! " + data.ExceptionMessage;
