ReportApp.controller('EstimationAvayaMasterController', function ($scope, $rootScope, _, EstimationAvayaMasterService, toaster, SelectionService, $timeout, EstimationPageSectionService) {
    ...
    $scope.deleteRow = function () {
        ...
        if ($scope.Servers.length > 1 && $scope.Servers[rowid].IsHeader != 'Y') {
            ...
            $scope.Grid.api.setRowData($scope.Servers);
        }
        else {
    ...
    $scope.addFreshrows = function () {
        ...
        EstimationPageSectionService.GetAllPageSections().success(function (data) {
            ...
            EstimationAvayaMasterService.GetAllEstimationAvayaMaster().success(function (data) {
    ...
    $scope.savemaster = function () {
        ...
        if (filtered != null) {
            EstimationAvayaMasterService.AddEstimationAvayaMaster(filtered).success(function (data) {
                if (data.Error != undefined) {
                    toaster.pop('error', "Error", data.Error, null);
                } else {

                    toaster.pop('success', "Success", "Estimation Avaya Master added successfully", null);
                }
            }).error(function (data) {
                toaster.pop('error', "Error", "An Error has occured while Adding Estimation Application Master!", null);
                $scope.error = "An Error has occured while Adding Estimation Application Master! " + data.ExceptionMessage;
