ReportApp.controller('EstimationAdminReportsMasterController', function ($scope, $rootScope, _, EstimationAdminReportsMasterService, toaster, SelectionService, $timeout, EstimationPageSectionService) {
    ...
    $scope.deleteRow = function () {
        ...
        if ($scope.AdminApplication.length > 1 && $scope.AdminApplication[rowid].IsHeader != 'Y') {
            $scope.AdminApplication.splice(rowid, 1)
            $scope.gridOptionsFixed.api.setRowData($scope.AdminApplication);
        }
        else {
    ...
    $scope.Addrow = function (GroupEditor, rowid) {
        $scope.AdminApplication.splice(rowid, 0, GroupEditor)
    }



    $scope.columnDefFirst1 = [
        { headerName: "RowId", field: "RowId", width: 50 },
        { headerName: "AdminApplication", field: "Name", width: 530, cellRenderer: TextEditor },
        { headerName: "", field: "col1", width: 60, cellRenderer: Editor },
        { headerName: "", field: "col2", width: 60, cellRenderer: Editor },
        { headerName: "Hours", field: "Fcol3", width: 440, cellRenderer: Editor },
    ];
    ...
    $scope.gridOptionsFixed = {
        ...
        columnDefs: [],
        groupHeaders: true,
        rowData: $scope.AdminApplication,
        debug: true,
        debug: true,
    ...
    $scope.lastselected = { 'Rowid': 1, 'Gridname': 'AdminApplication' };
    ...
    function callbackgrid() {
        $scope.columnDefFirst1[1].headerName = 'Admin Application';
        var local0 = angular.copy($scope.columnDefFirst1);
        $scope.gridOptionsFixed.api.setColumnDefs(local0);
    ...
    $scope.addFreshrows = function () {
        ...
        EstimationPageSectionService.GetAllPageSections().success(function (data) {
            ...
            EstimationAdminReportsMasterService.GetAllEstimationAdminReportsMaster().success(function (data) {
                if (data.Error != undefined) {
                    toaster.pop('error', "Error", data.Error, null);
                } else {

                    var filteredsections = _.filter($scope.GridSections, function (obj) {
                        return obj.PageName == "AdminReports";
                    })
    ...
    $scope.addnewpricerow = function () {
        ...
        var GroupType = '';
        var data = $scope.AdminApplication;
    ...
    $scope.savemaster = function () {
        ...
        if (filtered != null) {
            ...
            EstimationAdminReportsMasterService.AddEstimationAdminReportsMaster(filtered).success(function (data) {
                if (data.Error != undefined) {
                    toaster.pop('error', "Error", data.Error, null);
                } else {

                    toaster.pop('success', "Success", "Estimation Admin Reports Master added successfully", null);
                }
            }).error(function (data) {
                toaster.pop('error', "Error", "An Error has occured while Adding Estimation Application Master!", null);
                $scope.error = "An Error has occured while Adding Estimation Application Master! " + data.ExceptionMessage;
