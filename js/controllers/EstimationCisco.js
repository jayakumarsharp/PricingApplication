ReportApp.controller('EstimationCiscoController', function ($scope, $rootScope, _, EstimationCiscoMasterService, toaster, $routeParams, $timeout, Opportunityservice, $window, $location, EstimationApplicationMasterService, SelectionService, EstimationPageSectionService) {
    ...
    $scope.EditSheet = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var LockSheetModel = { OppId: $scope.OpportunityDetail.OppId, GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'CiscoSheet', IsPriceSheetUpdated: true };

            EstimationCiscoMasterService.LocktheSheetByGroupid(LockSheetModel).success(function (data) {
                ...
                if (!data.IsLocked) {
                    ...
                    if (data.LockedUser == $rootScope.UserInfo.user.userId && data.LockedIn == "CiscoSheet") {
                        $scope.hasedit = false;
                        $scope.griduseredit = true;
                        $scope.grideditable = true;
                        $routeParams.GroupId = data.ApplicationId;
    ...
    function getservicedata() {
        ...
        if ($routeParams.GroupId != 'undefined') {
            EstimationCiscoMasterService.GetAllEstimationCiscobyOppGroupID($scope.OpportunityDetail.OppId, $scope.MaxSheetGroupID).success(function (data) {
                if (data.Error == null) {
    ...
    function gettypebsedfill(params) {
        angular.forEach(params, function (val, key) {
            ...
            if (val.IsHeader != 'Y') {
                ...
                localJson['A1col1'] = val.col1;
                localJson['A1col2'] = val.col2;
                localJson['A1col3'] = val.col3;
                localJson['A1Fcol1'] = val.A1Fcol1;
                localJson['A1Fcol2'] = val.A1Fcol2;
                localJson['A1Fcol3'] = val.A1Fcol3;
            }
    ...
    var columnDefs = [
        {
            headerName: "", hide: true, field: "IsHeader", width: 1
        },
        {
        ...
        {
            headerName: "", field: "Name", width: 370
        },
        {
            headerName: 'PROD', field: "A1col1", width: 70, cellRenderer: NonPercentTypeEditor
        },
        {
            headerName: 'UAT', field: "A1col2", width: 70, cellRenderer: NonPercentTypeEditor
        },
        {
            headerName: 'UPG', field: "A1col3", width: 70, cellRenderer: NonPercentTypeEditor
        },

        {
            headerName: "", field: "Name1", width: 310
        },
        {
            headerName: 'PROD',
            field: "A1Fcol1", width: 70, cellRenderer: function (params) {
                if (params.value > 0) {
        ...
        {
            headerName: 'UAT',
            field: "A1Fcol2", width: 70, cellRenderer: function (params) {
                if (params.value > 0) {
                    return '<span title=' + params.value + '>' + parseFloat(params.value).toFixed(2) + '</span>';
                }
                else {
        ...
        {
            headerName: 'UPG',
            field: "A1Fcol3", width: 70, cellRenderer: function (params) {
                if (params.value > 0) {
                    return '<span title=' + params.value + '>' + parseFloat(params.value).toFixed(2) + '</span>';
                }
                else {
    ...
    function callbacksave(isdiscard) {
        ...
        var Jsondata = { OppId: $scope.OpportunityDetail.OppId, IsSaveAs: false, ApplicationId: $scope.MaxSheetGroupID, Version: $scope.MaxVersion, CiscoSheet: filtered, Authour: $rootScope.UserInfo.user.userId, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: $scope.Totalresult, RootId: $routeParams.OppId, Content: $scope.content };
    ...
    function AddSheetInternalcall(Jsondata) {
        EstimationCiscoMasterService.AddEstimationCisco(Jsondata).success(function (data) {
            ...
            if (data.Error == '' || data.Error == undefined || data.Error == null) {
                ...
                var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'CISCO', ApplicationId: $scope.MaxSheetGroupID, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
                ...
                EstimationApplicationMasterService.dointernalpercentagecalculationForCiscoGroup(IPdata).success(function (data) {
                    ...
                    if (!$scope.forceupdate)
                        toaster.pop('success', "SAVE", 'Cisco Sheet Saved Successfully', 3000);
                    return true;
    ...
    $scope.UpdatePriceSheetVersion = function () {
        ...
        EstimationApplicationMasterService.GetMaximumGroupEstimationApplicationId().success(function (data) {
            ...
            var Jsondata = { ExisitingPriceGroupId: $scope.MaxSheetGroupID, OppId: $scope.OpportunityDetail.OppId, IsSaveAs: true, ApplicationId: $scope.MaxSheetGroupIDForSaveAs, IsReadOnly: $scope.grideditable, CiscoSheet: filtered, Comment: $scope.Comment, Authour: $rootScope.UserInfo.user.userId };
            Jsondata.Version = version;
            Jsondata.NumberofApp = $scope.CurrentAdditionalGrid
            EstimationCiscoMasterService.AddEstimationCisco(Jsondata).success(function (data) {
                if (data.Error == '' || data.Error == undefined || data.Error == null) {
                    var newdata = $scope.Totalresult.concat($scope.result1);
                    var IPdata = { OppId: $scope.OpportunityDetail.OppId, ProductName: 'CISCO', ApplicationId: $scope.MaxSheetGroupIDForSaveAs, NumberofApp: $scope.CurrentAdditionalGrid, TotalRow: newdata, RootId: $routeParams.OppId };
    ...
    $scope.IncreaseAdditionalTimeToSheet = function () {
        var LockSheetModel = { GroupId: $scope.MaxSheetGroupID, username: $rootScope.UserInfo.user.userId, LockedInApp: 'CiscoSheet' };
        EstimationCiscoMasterService.IncreaseAdditionalTimeToSheet(LockSheetModel).success(function (data) {
            if (!data.IsLocked) {
                $scope.hasedit = false;
                $scope.griduseredit = true;
    ...
    $scope.IgnoreChanges = function () {
        EstimationCiscoMasterService.ReleaseSheetWhenExpired($scope.MaxSheetGroupID).success(function (data) {
            //alert(data)
            $scope.hasedit = true;
            $scope.griduseredit = false;
