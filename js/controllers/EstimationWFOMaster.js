ReportApp.controller('EstimationWFOMasterController', function ($scope, $rootScope, _, $timeout, EstimationWFOMasterService, toaster, SelectionService, EstimationPageSectionService) {
    ...
    $scope.deleteRow = function () {
        ...
        if ($scope.Nice.length > 1 && $scope.Nice[rowid].IsHeader != 'Y') {
            $scope.Nice.splice(rowid, 1)
            $scope.Grid.api.setRowData($scope.Nice);
        }
        else {
    ...
    $scope.Addrow = function (GroupEditor, rowid) {
        $scope.Nice.splice(rowid, 0, GroupEditor)
    }
    ...
    var columnDefFirst = [
        ...
        {
            headerName: "", field: "Name", width: 303, cellRenderer: TextEditor
        },
        {
    ...
    $scope.lastselected = { 'Rowid': 1, 'Gridname': 'Nice' };
    ...
    $scope.addFreshrows = function () {
        ...
        EstimationPageSectionService.GetAllPageSections().success(function (data) {
            ...
            EstimationWFOMasterService.GetAllEstimationWFOMaster().success(function (data) {
    ...
    $scope.savemaster = function () {
        ...
        if (filtered != null) {
            ...
            EstimationWFOMasterService.AddEstimationWFOMaster(filtered).success(function (data) {
                if (data.Error != undefined) {
                    toaster.pop('error', "Error", data.Error, null);
                } else {

                    toaster.pop('success', "Success", "Estimation WFO Master added successfully", null);
                }
            }).error(function (data) {
                toaster.pop('error', "Error", "An Error has occured while Adding Estimation Application Master!", null);
                $scope.error = "An Error has occured while Adding Estimation Application Master! " + data.ExceptionMessage;
